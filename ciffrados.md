...
Ataque MITM (Man-in-the-Middle): No protegía adecuadamente el handshake.

Falta de protección por truncamiento: Permitía ataques de truncamiento de mensajes.
...
OpenSSL ultima versión:  https://github.com/openssl/openssl/releases/
...
3DES , CVE-2016-2183 ,  CVE-2016-6329 (también conocido como "Sweet32") es una vulnerabilidad en los algoritmos de ci-frado DES/3DES que afecta a varias implementaciones de TLS y SSH. Es un tipo de ataque de cum-pleaños que permite colisiones cuando se usan cifrados débiles.
Ataque de Cumpleaños:  El servidor web cifra los datos mediante claves criptográficas. Estas claves se eligen aleatoriamente, y la probabilidad de que dos clientes obtengan la misma clave es muy baja. Al hacer un mal uso de la vulnerabilidad SWEET32 , un atacante puede enviar un gran volumen de datos ficticios y obtener bloques de texto cifrado que coinciden con los de un cliente  https://sweet32.info/ 
...
RSA Key, el ataque mas facil y peligros es el de interceptación de clave RSA ("Man-in-the-Middle" o MiTM en el intercambio de claves), con herramientas como mitmproxy/mitmproxy: An interactive TLS-capable intercepting HTTP proxy for penetration testers and software developers.   https://github.com/mitmproxy/mitmproxy 
